...
func main() {
	fmt.Println("Blend two JSONLD docs together")

	s, err := JLDArray(examplePerson(), examplePerson())
	if err != nil {
...
func JLDList(v ...string) (string, error) {

	// really this is the same as JLD array..
	// it can be done with a simple text array..   one could then
	// vet it as valid JSON-LD before returning it though to ensure
	// no encoding issues.

	return "list", nil
}

func JLDArray(v ...string) (string, error) {
