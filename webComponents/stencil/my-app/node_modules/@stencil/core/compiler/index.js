...
function generateComponentModules(config, ctx, manifestBundle) {
    var bundleCacheKey = getModuleBundleCacheKey(manifestBundle.moduleFiles.map(function (m) { return m.cmpMeta.tagNameMeta; }));
    ...
    var moduleBundleInput = createInMemoryBundleInput(manifestBundle.moduleFiles).join('\n');
...
function createInMemoryBundleInput(moduleFiles) {
    ...
    moduleFiles.sort(function (a, b) {
        ...
        var asName = dashToPascalCase(moduleFile.cmpMeta.tagNameMeta);
...
function canSkipBuild(config, ctx, moduleFiles, cacheKey) {
    ...
    var bundleContainsChangedFile = bundledComponentContainsChangedFile(config, moduleFiles, ctx.changedFiles);
...
function bundledComponentContainsChangedFile(config, bundlesModuleFiles, changedFiles) {
    ...
    return bundlesModuleFiles.some(function (moduleFile) {
        var distFileName = config.sys.path.basename(moduleFile.jsFilePath, '.js');
        return changedFiles.some(function (f) {
            var changedFileName = config.sys.path.basename(f);
            return (changedFileName === distFileName + '.ts' || changedFileName === distFileName + '.tsx');
        });
    });
}
function getModuleBundleCacheKey(components) {
    return components.map(function (c) { return c.toLocaleLowerCase().trim(); }).sort().join('.');
}
var IN_MEMORY_INPUT = '__IN_MEMORY_INPUT__';
...
function bundleModules(config, ctx, manifestBundles) {
    ...
    return Promise.all(manifestBundles.map(function (manifestBundle) {
...
function scopeComponentCss(ctx, cmpMeta, cssText) {
    try {
        var scopeAttribute = getScopeAttribute(cmpMeta);
        var hostScopeAttr = getHostScopeAttribute(cmpMeta);
...
function scopeCss(cssText, scopeAttribute, hostScopeAttr) {
...
function generateComponentStyles(config, ctx, moduleFile) {
    var modes = Object.keys(moduleFile.cmpMeta.stylesMeta);
    var promises = modes.map(function (modeName) {
        return generateComponentModeStyles(config, ctx, moduleFile, modeName);
    });
    return Promise.all(promises).then(function (componentModeStyles) {
        if (componentModeStyles.some(function (c) { return c.writeFile; })) {
...
function generateComponentModeStyles(config, ctx, moduleFile, modeName) {
    return generateAllComponentModeStyles(config, ctx, moduleFile, modeName).then(function (allCmpStyleDetails) {
        var compiledModeStyles = groupComponentModeStyles(moduleFile.cmpMeta.tagNameMeta, modeName, allCmpStyleDetails);
...
function generateAllComponentModeStyles(config, ctx, moduleFile, modeName) {
    ...
    if (modeStyleMeta) {
        ...
        if (modeStyleMeta.absolutePaths) {
            modeStyleMeta.absolutePaths.forEach(function (absStylePath) {
                ...
                if (isSassFile(absStylePath)) {
                    ...
                    promises.push(compileSassFile(config, ctx, moduleFile, absStylePath, styleOrder_1));
                }
                else if (isCssFile(absStylePath)) {
                    // plain ol' css file
                    promises.push(readCssFile(config, ctx, moduleFile.cmpMeta, absStylePath, styleOrder_1));
                }
                else {
                    // idk
        ...
        if (typeof modeStyleMeta.styleStr === 'string') {
            ...
            promises.push(readInlineStyles(config, ctx, moduleFile.cmpMeta, modeStyleMeta.styleStr, styleOrder_1));
        }
    }
    return Promise.all(promises);
}
function groupComponentModeStyles(tag, modeName, allCmpStyleDetails) {
    var compiledModeStyles = {
        tag: tag,
        modeName: modeName,
        writeFile: allCmpStyleDetails.some(function (c) { return c.writeFile; })
    };
    if (allCmpStyleDetails.length === 0) {
        return compiledModeStyles;
    }
    allCmpStyleDetails = allCmpStyleDetails.sort(function (a, b) {
    ...
    compiledModeStyles.unscopedStyles = allCmpStyleDetails.map(function (s) { return s.unscopedStyles || ''; }).join('\n\n').trim();
    // group all scoped css
    compiledModeStyles.scopedStyles = allCmpStyleDetails.map(function (s) { return s.scopedStyles || ''; }).join('\n\n').trim();
    return compiledModeStyles;
}
function setHydratedCss(config, cmpMeta, compiledModeStyles) {
    var tagSelector = cmpMeta.tagNameMeta + "." + config.hydratedCssClass;
    if (cmpMeta.encapsulation === 1 /* ShadowDom */) {
        var hostSelector = ":host(." + config.hydratedCssClass + ")";
...
function compileSassFile(config, ctx, moduleFile, absStylePath, styleOrder) {
    ...
    if (ctx.isChangeBuild && !ctx.changeHasSass) {
        ...
        var distFileName_1 = config.sys.path.basename(moduleFile.jsFilePath, '.js');
    ...
    return new Promise(function (resolve) {
        var sassConfig = __assign({}, config.sassConfig, { file: absStylePath, outputStyle: config.minifyCss ? 'compressed' : 'expanded' });
        config.sys.sass.render(sassConfig, function (err, result) {
            if (err) {
                var d = buildError(ctx.diagnostics);
...
function fillStyleText(config, ctx, cmpMeta, compiledModeStyles, unscopedStyles) {
    ...
    if (typeof unscopedStyles === 'string') {
        ...
        if (config.minifyCss) {
            // minify css
            var minifyCssResults = config.sys.minifyCss(compiledModeStyles.unscopedStyles);
...
function generateBundleComponentStyles(config, ctx, manifestBundle) {
    var moduleFilesWithStyles = getModuleFilesWithStyles(manifestBundle.moduleFiles);
    var promises = moduleFilesWithStyles.map(function (moduleFile) {
        return generateComponentStyles(config, ctx, moduleFile).then(function (compiledModeStyles) {
            (_a = manifestBundle.compiledModeStyles).push.apply(_a, compiledModeStyles);
            var _a;
        });
    });
    return Promise.all(promises);
...
function getModuleFilesWithStyles(allModuleFiles) {
    return allModuleFiles
        .filter(function (m) { return m.cmpMeta && m.cmpMeta.stylesMeta && Object.keys(m.cmpMeta.stylesMeta).length; })
        .sort(function (a, b) {
...
function formatBundleIds(bundleIds) {
    ...
    var modes = Object.keys(bundleIds).sort();
    if (!modes.length) {
        return 'invalid-bundle-id';
    }
    if (modes.length === 1) {
...
function formatLoadStyles(namespace, bundleStyles, scoped) {
    var args = [];
    bundleStyles = bundleStyles.sort(function (a, b) {
        if (a.tag < b.tag)
            return -1;
        if (a.tag > b.tag)
            return 1;
    ...
    bundleStyles.forEach(function (bundleStyle) {
        var styles = (scoped ? bundleStyle.scopedStyles : bundleStyle.unscopedStyles).replace(/\n/g, "\\n").replace(/\"/g, "\\\"").trim();
        if (styles.length > 0) {
            // arg EVEN
            var styleId = bundleStyle.tag;
...
function getAppPublicPath(config) {
    return normalizePath(config.sys.path.join(config.publicPath, getAppFileName(config))) + '/';
}
    ...
    return generateAppGlobal(config, ctx).then(function (globalJsContents) {
        ...
        if (ctx.appFiles.core !== coreContent) {
            ...
            if (config.generateWWW) {
                ...
                var appCoreWWW = normalizePath(sys.path.join(config.buildDir, appFileName, appRegistry.core));
                ctx.filesToWrite[appCoreWWW] = coreContent;
            }
            if (config.generateDistribution) {
                // write the dist/ app core file
                var appCoreDist = normalizePath(sys.path.join(config.distDir, appFileName, appRegistry.core));
        ...
        if (ctx.appFiles.corePolyfilled !== coreEs5WithPolyfilledContent) {
            ...
            if (config.generateWWW) {
                ...
                var appCorePolyfilledWWW = normalizePath(sys.path.join(config.buildDir, appFileName, appRegistry.corePolyfilled));
                ctx.filesToWrite[appCorePolyfilledWWW] = coreEs5WithPolyfilledContent;
            }
            if (config.generateDistribution) {
                // write the dist app core polyfilled file
                var appCorePolyfilledDist = normalizePath(sys.path.join(config.distDir, appFileName, appRegistry.corePolyfilled));
        ...
        return generateLoader(config, appRegistry.core, appRegistry.corePolyfilled, appRegistry.components).then(function (loaderContent) {
            ...
            if (ctx.appFiles.loader !== loaderContent) {
                ...
                if (config.generateWWW) {
                    var appLoaderWWW = normalizePath(sys.path.join(config.buildDir, appLoader));
                    ctx.filesToWrite[appLoaderWWW] = loaderContent;
                }
                if (config.generateDistribution) {
                    var appLoaderDist = normalizePath(sys.path.join(config.distDir, appLoader));
...
function generateBundles(config, ctx, manifestBundles) {
    manifestBundles.forEach(function (manifestBundle) {
        generateBundleFiles(config, ctx, manifestBundle);
    });
    ctx.registry = generateComponentRegistry(manifestBundles);
}
function generateBundleFiles(config, ctx, manifestBundle) {
    manifestBundle.moduleFiles.forEach(function (moduleFile) {
        moduleFile.cmpMeta.bundleIds = {};
    });
    var moduleText = formatLoadComponents(config.namespace, MODULE_ID, manifestBundle.compiledModuleText, manifestBundle.moduleFiles);
    if (config.minifyJs) {
        // minify js
        var minifyJsResults = config.sys.minifyJs(moduleText);
        minifyJsResults.diagnostics.forEach(function (d) {
            ctx.diagnostics.push(d);
        });
        if (!minifyJsResults.diagnostics.length) {
    ...
    var modes = getManifestBundleModes(manifestBundle.moduleFiles);
    var hasDefaultMode = containsDefaultMode(modes);
    var hasNonDefaultModes = containsNonDefaultModes(modes);
    if (hasDefaultMode && hasNonDefaultModes) {
        modes.filter(function (m) { return m !== DEFAULT_STYLE_MODE; }).forEach(function (modeName) {
            var bundleStyles = manifestBundle.compiledModeStyles.filter(function (cms) { return cms.modeName === DEFAULT_STYLE_MODE; });
            bundleStyles.push.apply(bundleStyles, manifestBundle.compiledModeStyles.filter(function (cms) { return cms.modeName === modeName; }));
            writeBundleFile(config, ctx, manifestBundle, moduleText, modeName, bundleStyles);
        });
    }
    else if (modes.length) {
        modes.forEach(function (modeName) {
            var bundleStyles = manifestBundle.compiledModeStyles.filter(function (cms) { return cms.modeName === modeName; });
            writeBundleFile(config, ctx, manifestBundle, moduleText, modeName, bundleStyles);
        });
    }
    else {
...
function writeBundleFile(config, ctx, manifestBundle, moduleText, modeName, bundleStyles) {
    var unscopedStyleText = formatLoadStyles(config.namespace, bundleStyles, false);
    var unscopedContents = [
        generatePreamble(config)
    ];
    ...
    var unscopedContent = unscopedContents.join('\n');
    var bundleId = getBundleId(config, manifestBundle.moduleFiles.map(function (m) { return m.cmpMeta.tagNameMeta; }), modeName, unscopedContent);
    unscopedContent = replaceDefaulBundleId(unscopedContent, bundleId);
    var unscopedFileName = getBundleFileName(bundleId, false);
    setBundleModeIds(manifestBundle.moduleFiles, modeName, bundleId);
    var unscopedWwwBuildPath = normalizePath(config.sys.path.join(config.buildDir, getAppFileName(config), unscopedFileName));
    ...
    if (config.generateDistribution) {
        ...
        var unscopedDistPath = normalizePath(config.sys.path.join(config.distDir, getAppFileName(config), unscopedFileName));
        ctx.filesToWrite[unscopedDistPath] = unscopedContent;
    }
    if (modeName && bundleRequiresScopedStyles(manifestBundle.moduleFiles)) {
        var scopedStyleText = formatLoadStyles(config.namespace, bundleStyles, true);
        var scopedContents = [
            generatePreamble(config)
        ];
        ...
        var scopedFileContent = replaceDefaulBundleId(scopedContents.join('\n'), bundleId);
        var scopedFileName = getBundleFileName(bundleId, true);
        if (config.generateWWW) {
            // write the scoped css to the www build
            var scopedWwwPath = normalizePath(config.sys.path.join(config.buildDir, getAppFileName(config), scopedFileName));
            ctx.filesToWrite[scopedWwwPath] = scopedFileContent;
        }
        if (config.generateDistribution) {
            // write the scoped css to the dist build
            var scopedDistPath = normalizePath(config.sys.path.join(config.distDir, getAppFileName(config), scopedFileName));
            ctx.filesToWrite[scopedDistPath] = scopedFileContent;
        }
    }
}
function setBundleModeIds(moduleFiles, modeName, bundleId) {
...
function generateComponentRegistry(manifestBundles) {
    ...
    var registry = {};
    manifestBundles.forEach(function (manifestBundle) {
        manifestBundle.moduleFiles.filter(function (m) { return m.cmpMeta; }).forEach(function (moduleFile) {
            registryComponents.push(moduleFile.cmpMeta);
        });
    });
    registryComponents.sort(function (a, b) {
...
function getManifestBundleModes(moduleFiles) {
    ...
    moduleFiles.forEach(function (m) {
        if (m.cmpMeta && m.cmpMeta.stylesMeta) {
            Object.keys(m.cmpMeta.stylesMeta).forEach(function (modeName) {
...
function bundleRequiresScopedStyles(moduleFiles) {
    return moduleFiles
        .filter(function (m) { return m.cmpMeta && m.cmpMeta.stylesMeta; })
        .some(function (m) { return componentRequiresScopedStyles(m.cmpMeta.encapsulation); });
}
function containsDefaultMode(modes) {
    return modes.some(function (m) { return m === DEFAULT_STYLE_MODE; });
}
function containsNonDefaultModes(modes) {
    return modes.length > 0 && modes.some(function (m) { return m !== DEFAULT_STYLE_MODE; });
}
function replaceDefaulBundleId(fileContent, newModuleId) {
    return fileContent.replace(MODULE_ID_REGEX, newModuleId);
}
var MODULE_ID = '__STENCIL__MODULE__ID__';
var MODULE_ID_REGEX = new RegExp(MODULE_ID, 'g');
...
function bundle(config, ctx) {
    ...
    var manifestBundles = getManifestBundles(ctx.manifest.modulesFiles, ctx.manifest.bundles, ctx.diagnostics);
...
function getManifestBundles(moduleFiles, bundles, diagnostics) {
    var manifestBundles = [];
    bundles.filter(function (b) { return b.components && b.components.length; }).forEach(function (bundle) {
        var manifestBundle = createManifestBundle([], bundle.priority);
        bundle.components.forEach(function (tag) {
            var cmpMeta = moduleFiles.find(function (modulesFile) { return modulesFile.cmpMeta.tagNameMeta === tag; });
            if (cmpMeta) {
...
function validateManifestBundle(validatedBundles, manifestBundle) {
    ...
    var primaryEncapsulation = findPrimaryEncapsulation(manifestBundle.moduleFiles);
...
function findPrimaryEncapsulation(moduleFiles) {
    var plainCssCmps = moduleFiles.filter(function (m) { return m.cmpMeta.encapsulation !== 2 /* ScopedCss */ && m.cmpMeta.encapsulation !== 1 /* ShadowDom */; });
    var scopedCssCmps = moduleFiles.filter(function (m) { return m.cmpMeta.encapsulation === 2 /* ScopedCss */; });
    var shadowCssCmps = moduleFiles.filter(function (m) { return m.cmpMeta.encapsulation === 1 /* ShadowDom */; });
    // figure out which encapsulation type we have the most of
    var sorted = [plainCssCmps, scopedCssCmps, shadowCssCmps].sort(function (a, b) {
...
function sortBundles(manifestBundles) {
    manifestBundles.forEach(function (m) {
        m.moduleFiles = m.moduleFiles.sort(function (a, b) {
            if (a.cmpMeta.tagNameMeta < b.cmpMeta.tagNameMeta)
                return -1;
            if (a.cmpMeta.tagNameMeta > b.cmpMeta.tagNameMeta)
                return 1;
    ...
    return manifestBundles.sort(function (a, b) {
...
function transformSourceString(fileName, sourceText, transformers) {
    var transformed = ts.transform(ts.createSourceFile(fileName, sourceText, ts.ScriptTarget.ES2015), transformers);
...
function serializeStyles(config, moduleFile, compiledComponentRelativeDirPath, cmpData, cmpMeta) {
    if (cmpMeta.stylesMeta) {
        cmpData.styles = {};
        var modeNames = Object.keys(cmpMeta.stylesMeta).map(function (m) { return m.toLowerCase(); }).sort();
        ...
        if (isProp && propName) {
            ...
            if (userPropOptions) {
                ...
                if (typeof userPropOptions.mutable === 'boolean') {
                    propMeta.memberType = 2 /* PropMutable */;
                }
            }
...
function visitClass(config, moduleFile, diagnostics, classNode) {
    ...
    moduleFile.cmpMeta = __assign$2({}, cmpMeta, { componentClass: classNode.name.getText().trim(), membersMeta: __assign$2({}, getElementDecoratorMeta(classNode), getMethodDecoratorMeta(classNode), getStateDecoratorMeta(classNode), getPropDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode)), eventsMeta: getEventDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode), listenersMeta: getListenDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode) }, getPropChangeDecoratorMeta(classNode));
...
function membersToInterfaceOptions(membersMeta) {
    var memberTypes = (_a = {},
        _a[1 /* Any */] = 'any',
        _a[2 /* String */] = 'string',
        _a[3 /* Boolean */] = 'boolean',
        _a[4 /* Number */] = 'number',
        _a);
    return Object.keys(membersMeta)
        .filter(function (memberName) {
        return [6 /* Method */, 1 /* Prop */, 4 /* PropConnect */, 2 /* PropMutable */].indexOf(membersMeta[memberName].memberType) !== -1;
    })
        .reduce(function (obj, memberName) {
        var member = membersMeta[memberName];
        obj[memberName] = memberTypes[member.propType || 1 /* Any */];
        return obj;
    }, {});
    var _a;
}
...
function getIncludedSassFiles(config, diagnostics, moduleFile, scssFilePath) {
    return new Promise(function (resolve) {
        ...
        var sassConfig = __assign$1({}, config.sassConfig, { file: scssFilePath });
...
function generate(config, ctx) {
    ...
    return config.sys.workbox.generateSW(config.serviceWorker)
        .catch(function (err) { return catchError(ctx.diagnostics, err); })
        .then(function () { return timeSpan.finish("generate service worker finished"); });
}
function injectManifest(config, ctx) {
    var timeSpan = config.logger.createTimeSpan("inject manifest into service worker started");
    return config.sys.workbox.injectManifest(config.serviceWorker)
        .catch(function (err) { return catchError(ctx.diagnostics, err); })
        .then(function () { return timeSpan.finish('inject manifest into service worker finished'); });
}
function hasSrcConfig(config) {
    var serviceWorkerConfig = config.serviceWorker;
...
function loadDependentManifest(config, ctx, dependentCollection) {
    ...
    var dependentPackageJsonFilePath = config.sys.resolveModule(config.rootDir, dependentCollection.name);
    // parse the dependent collection's package.json
    var packageData = JSON.parse(config.sys.fs.readFileSync(dependentPackageJsonFilePath, 'utf-8'));
    ...
    var dependentPackageRootDir = config.sys.path.dirname(dependentPackageJsonFilePath);
    // figure out the full path to the collection manifest file
    var dependentManifestFilePath = normalizePath(config.sys.path.join(dependentPackageRootDir, packageData.collection));
...
function validatePackageJson(config, diagnostics, data) {
    ...
    var browser = normalizePath(config.sys.path.join(config.sys.path.relative(config.rootDir, config.distDir), getAppFileName(config) + '.js'));
...
function generatePackageModuleResolve(config) {
    return __awaiter(this, void 0, void 0, function () {
        var PromiseList, packageResolver, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    PromiseList = [];
                    packageResolver = config.sys.path.join(config.collectionDir, 'index.js');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, config.sys.ensureFile(config.sys.path.join(config.srcDir, 'index.d.ts'))];
                case 2:
                    _a.sent();
...
function validateManifestCompatibility(config, manifest) {
    ...
    try {
        ...
        // parse version
        var manifestCompilerVersion = parseCompilerVersion(manifest.compiler.version);
...
function parseCompilerVersion(version) {
    var dashSplt = version.split('-');
    var dotSplt = dashSplt[0].split('.');
    if (dotSplt.length !== 3) {
    ...
    var prereleaseSplt = dashSplt.slice(1);
    if (prereleaseSplt.length > 1) {
        throw new Error("invalid semver: " + version);
    }
    if (prereleaseSplt.length === 1) {
        var prerelease = parseInt(prereleaseSplt[0], 10);
        if (isNaN(prerelease)) {
            throw new Error("invalid semver: " + version);
        }
        else {
...
function semverLessThan(a, b) {
    var semverA = (typeof a === 'string') ? parseCompilerVersion(a) : a;
    var semverB = (typeof b === 'string') ? parseCompilerVersion(b) : b;
...
function upgradeProps(props) {
    ...
    var objectProps = objectLiteralToObjectMap(props);
    upgradedProps = Object.keys(objectProps).reduce(function (newProps, propName) {
        var propValue = objectProps[propName];
        // If the propname is c, s, or k then map to proper name
        if (propName === 'c') {
            return __assign$3({}, newProps, { 'class': propValue });
        }
        if (propName === 's') {
            return __assign$3({}, newProps, { 'style': propValue });
        }
        if (propName === 'k') {
            return __assign$3({}, newProps, { 'key': propValue });
        }
        // If the propname is p or a then spread the value into props
        if (propName === 'a') {
            return __assign$3({}, newProps, propValue);
        }
        if (propName === 'p') {
        ...
        if (propName === 'o') {
            var eventListeners = Object.keys(propValue).reduce(function (newValue, eventName) {
                return __assign$3({}, newValue, (_a = {}, _a["on" + eventName] = propValue[eventName], _a));
                var _a;
            }, {});
            return __assign$3({}, newProps, eventListeners);
        }
        return newProps;
    }, upgradedProps);
    try {
...
function upgradeDependentComponents(config, ctx) {
    return __awaiter$1(this, void 0, void 0, function () {
        ...
        return __generator$1(this, function (_a) {
            doUpgrade = createDoUpgrade(config, ctx);
            return [2 /*return*/, Promise.all(Object.keys(ctx.dependentManifests).map(function (collectionName) {
                    return __awaiter$1(this, void 0, void 0, function () {
                        var manifest, upgrades, e_1;
                        return __generator$1(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    manifest = ctx.dependentManifests[collectionName];
                                    upgrades = validateManifestCompatibility(config, manifest);
                                    _a.label = 1;
                                case 1:
                                    _a.trys.push([1, 3, , 4]);
                                    return [4 /*yield*/, doUpgrade(manifest, upgrades)];
                                case 2:
                                    _a.sent();
                                    return [3 /*break*/, 4];
                                case 3:
                                    e_1 = _a.sent();
                                    config.logger.error("error performing compiler upgrade: " + e_1);
                                    return [3 /*break*/, 4];
...
function createDoUpgrade(config, ctx) {
    var _this = this;
    return function (manifest, upgrades) { return __awaiter$1(_this, void 0, void 0, function () {
        var upgradeTransforms;
        return __generator$1(this, function (_a) {
            switch (_a.label) {
                case 0:
                    upgradeTransforms = (upgrades.map(function (upgrade) {
                        switch (upgrade) {
                        ...
                        return function () { return function (tsSourceFile) { return (tsSourceFile); }; };
                    }));
                    if (upgradeTransforms.length === 0) {
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, Promise.all(manifest.modulesFiles.map(function (moduleFile) {
                            return __awaiter$1(this, void 0, void 0, function () {
                                return __generator$1(this, function (_a) {
                                    return [2 /*return*/, new Promise(function (resolve, reject) {
                                            config.sys.fs.readFile(moduleFile.jsFilePath, 'utf8', function (err, source) {
                                                if (err) {
                                                    reject(err);
                                                }
                                                else {
